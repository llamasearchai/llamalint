...
app = typer.Typer(
    help="LlamaLint - Code linting and formatting tools for LlamaSearch.ai projects"
)
console = Console()
...
def lint(
    ctx: typer.Context,
    paths: List[str] = typer.Argument(None, help="Files or directories to lint"),
    fix: bool = typer.Option(
        False, "--fix", help="Automatically fix issues where possible"
    ),
    ignore: List[str] = typer.Option([], "--ignore", "-i", help="Rules to ignore"),
    ...
    try:
        ...
        if output_format == "json":
            ...
            if not all_results:
                ...
                console.print(
                    f"Found [bold red]{error_count}[/] errors and [bold yellow]{warning_count}[/] warnings"
                )
                ...
                for result in all_results:
                    ...
                    console.print(
                        f"  [dim]{result.file_path}:{result.line}:{result.column}[/]"
                    )
                    ...
                    if result.source:
                        syntax = Syntax(
                            ...
                            "python",
                            line_numbers=True,
                            start_line=max(1, result.line - 2),
                            highlight_lines=[result.line],
                        )
    ...
    try:
        ...
        if rule.options:
            ...
            for name, option in rule.options.items():
                ...
                if option.choices:
                    console.print(
                        f"    Choices: {', '.join(str(c) for c in option.choices)}"
                    )
