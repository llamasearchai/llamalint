...
class FileUtils:
    ...
    def find_files(
        ...
        for root, dirs, files in os.walk(directory):
            ...
            dirs[:] = [d for d in dirs if not any(r.search(str(root_path / d).replace("\\", "/")) for r in exclude_regexes)]
            ...
            for file in files:
                ...
                if (any(r.search(rel_path_str) for r in include_regexes) and
                    not any(r.search(rel_path_str) for r in exclude_regexes)):
