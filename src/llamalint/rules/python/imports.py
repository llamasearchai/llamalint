...
class ImportsRule(Rule):
    ...
    def check(self, file_path: Path, content: str) -> List[RuleResult]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.Import, ast.ImportFrom)):
                ...
                if isinstance(node, ast.ImportFrom) and node.level > 0 and self.get_option("require_absolute_imports"):
                    results.append(RuleResult(
                        rule_id=self.id,
                        file_path=str(file_path),
                        line=node.lineno,
                ...
                if len(lines[node.lineno - 1]) > self.get_option("max_import_line_length"):
                    results.append(RuleResult(
                        rule_id=self.id,
                        file_path=str(file_path),
                        line=node.lineno,
            ...
            for import_node in imports[1:]:
                ...
                if not self._is_proper_order(prev_import, import_node):
                    results.append(RuleResult(
                        ...
                        severity=self.severity,
                        source="\n".join(lines[prev_import.lineno - 1:import_node.lineno])
                    ))
            ...
            for node in imports:
                if isinstance(node, ast.Import):
                    for name in node.names:
                        ...
                        if imported_name not in used_names and imported_name in imported_names:
                            results.append(RuleResult(
                                rule_id=self.id,
                                file_path=str(file_path),
                                line=node.lineno,
    ...
    def _is_proper_order(self, prev_import: ast.AST, current_import: ast.AST) -> bool:
        ...
        if isinstance(current_import, ast.ImportFrom) and current_import.module == "__future__":
            if not (isinstance(prev_import, ast.ImportFrom) and prev_import.module == "__future__"):
                return False
                
        # Standard library imports > third-party imports > local imports
        if isinstance(prev_import, ast.Import) and isinstance(current_import, ast.Import):
            # Both are 'import' statements
            if prev_import.names[0].name.split(".")[0] > current_import.names[0].name.split(".")[0]:
                return False
        elif isinstance(prev_import, ast.ImportFrom) and isinstance(current_import, ast.ImportFrom):
            # Both are 'from' statements
            if prev_import.module and current_import.module:
                if prev_import.module.split(".")[0] > current_import.module.split(".")[0]:
                    return False
        elif isinstance(prev_import, ast.Import) and isinstance(current_import, ast.ImportFrom):
            # 'import' statements should come before 'from' statements for the same module
            if prev_import.names[0].name.split(".")[0] == current_import.module.split(".")[0]:
                return False
