...
class DocstringsRule(Rule):
    ...
    def check(self, file_path: Path, content: str) -> List[RuleResult]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                self._check_class_docstring(node, content, file_path, line_offsets, results)
            elif isinstance(node, ast.FunctionDef):
                self._check_function_docstring(node, content, file_path, line_offsets, results)
        ...
        if not docstring:
            ...
            if style_issues and self.get_option("style") != "any":
                results.append(RuleResult(
                    ...
                    severity=self.severity,
                    source=self._get_source_lines(content, node.lineno, node.lineno + 3)
                ))
    ...
    def _check_function_docstring(
        ...
        if not docstring:
            ...
            docstring_lines = [f'{indentation}    """Function description.', f'{indentation}    ']
    ...
    def _should_ignore_function(self, node: ast.FunctionDef) -> bool:
        ...
        if self.get_option("ignore_private") and node.name.startswith("_") and not node.name.startswith("__"):
            return True
            
        # Check if it's a dunder method and we're ignoring those
        if self.get_option("ignore_dunder") and node.name.startswith("__") and node.name.endswith("__"):
            return True
    ...
    def _check_docstring_style(self, docstring: str, node_type: str) -> List[str]:
        ...
        if style == "google":
            ...
            if node_type == "function" and "Args:" not in docstring and "Parameters:" not in docstring:
                issues.append("missing Args/Parameters section")
                
            if node_type == "function" and "Returns:" not in docstring and "Yields:" not in docstring:
