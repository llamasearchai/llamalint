...
class JavaScriptFormatter(Formatter):
    ...
    def _apply_prettier(self, content: str, file_path: Path) -> str:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=file_path.suffix, mode="w+", delete=False) as temp:
                temp.write(content)
                temp_path = temp.name
            ...
            try:
                ...
                prettier_cmd.append("--single-quote" if single_quote else "--no-single-quote")
                prettier_cmd.extend(["--trailing-comma", trailing_comma])
                prettier_cmd.append("--bracket-spacing" if bracket_spacing else "--no-bracket-spacing")
